...
BRIDGE_ENV_FILE="${REMOTE_BRIDGE_DIR}/.env"
...
expect "${PROJECT_DIR}/scripts/ssh_command.expect" \
  ...
  "printf '%s\n' '${ESCAPED_PASSWORD}' | sudo -S -p '' bash -lc 'if [ -f \"${VOICE_ENV_FILE}\" ]; then \
   ...
   cd \"${REMOTE_BRIDGE_DIR}/voice_services\" && ./configure_voice_services_env.sh && (docker compose stop whisper >/dev/null 2>&1 || true) && (docker compose rm -f whisper >/dev/null 2>&1 || true) && docker compose up -d --remove-orphans openwakeword speech-to-phrase piper; \

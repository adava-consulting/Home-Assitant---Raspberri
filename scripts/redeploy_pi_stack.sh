...
expect "${PROJECT_DIR}/scripts/ssh_command.expect" \
  ...
  "$HOST" \
   ...
   if [ -f \"${WAKEWORD_ENV_FILE}\" ]; then \
     ...
     if grep -q \"^WHISPER_BEAM_SIZE=\" \"${WAKEWORD_ENV_FILE}\"; then sed -i \"s/^WHISPER_BEAM_SIZE=.*/WHISPER_BEAM_SIZE=1/\" \"${WAKEWORD_ENV_FILE}\"; else printf \"\\nWHISPER_BEAM_SIZE=1\\n\" >> \"${WAKEWORD_ENV_FILE}\"; fi; \
     if grep -q \"^WHISPER_VAD_MIN_SPEECH_MS=\" \"${WAKEWORD_ENV_FILE}\"; then sed -i \"s/^WHISPER_VAD_MIN_SPEECH_MS=.*/WHISPER_VAD_MIN_SPEECH_MS=200/\" \"${WAKEWORD_ENV_FILE}\"; else printf \"\\nWHISPER_VAD_MIN_SPEECH_MS=200\\n\" >> \"${WAKEWORD_ENV_FILE}\"; fi; \
     if grep -q \"^WHISPER_VAD_MIN_SILENCE_MS=\" \"${WAKEWORD_ENV_FILE}\"; then sed -i \"s/^WHISPER_VAD_MIN_SILENCE_MS=.*/WHISPER_VAD_MIN_SILENCE_MS=700/\" \"${WAKEWORD_ENV_FILE}\"; else printf \"\\nWHISPER_VAD_MIN_SILENCE_MS=700\\n\" >> \"${WAKEWORD_ENV_FILE}\"; fi; \
     if grep -q \"^WHISPER_INITIAL_PROMPT=\" \"${WAKEWORD_ENV_FILE}\"; then sed -i \"s#^WHISPER_INITIAL_PROMPT=.*#WHISPER_INITIAL_PROMPT=Turn on the lights. Turn off the lights. Turn all lights on. Turn all lights off. Turn on the room lights. Turn off the room lights. Turn on the studio lights. Turn off the studio lights. Set the studio lights to blue. Set the studio lights to 50 percent brightness.#\" \"${WAKEWORD_ENV_FILE}\"; else printf \"\\nWHISPER_INITIAL_PROMPT=Turn on the lights. Turn off the lights. Turn all lights on. Turn all lights off. Turn on the room lights. Turn off the room lights. Turn on the studio lights. Turn off the studio lights. Set the studio lights to blue. Set the studio lights to 50 percent brightness.\\n\" >> \"${WAKEWORD_ENV_FILE}\"; fi; \
     cd \"${REMOTE_BRIDGE_DIR}/voice_services\" && docker compose up -d openwakeword whisper; \
   fi; \
